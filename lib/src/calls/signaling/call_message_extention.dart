...
class CallExtraParamsElement extends ExtraParamsElement {
  ...
  void setOpponents(Set<int> opponentsIds) {
    CubeXmppElement opponentsElement = CubeXmppElement();
    opponentsElement.name = SignalField.OPPONENTS;

    for (int opponentId in opponentsIds) {
      CubeXmppElement child = CubeXmppElement();
  ...
  List<RTCIceCandidate> getIceCandidates() {
    List<RTCIceCandidate> result = List();
  ...
  RTCIceCandidate getIceCandidate() {
    ...
    if (candidateElement == null &&
        sdpMidElement == null &&
        sdpMLineIndexElement == null) return null;

    String candidate = candidateElement.textValue;
    String sdpMid = sdpMidElement.textValue;
    int sdpMLineIndex = int.parse(sdpMLineIndexElement.textValue);
...
class CubeXmppElement extends XmppElement {
  @override
  String buildXmlString() {
    String string = buildXml().toXmlString(pretty: false);
    return string;
